    ...
    <>
      <div className="score-summary">
        <div className="summary-pct-wrap">
          ...
          <span className="summary-note">(キーワード正答率による概算 ○×2点 △×1点)</span>
        </div>
      ...
      {results.map((p) => (
        <div key={p.id} className="score-problem">
          <h2 className="score-problem-title">問{p.id}：{p.title}</h2>
              ...
              {group.questions.map((q) => {
                if (q.subQuestions && q.questions) {
                  return q.questions.map((subQ) => (
                    <div key={subQ.id} className={`score-question ${markClass(subQ.mark)}`}>
                      <div className="sq-header">
                        <span className="sq-label">{subQ.label}</span>
                        <span className={`sq-mark ${markClass(subQ.mark)}`}>{subQ.mark}</span>
                        <span className="sq-limit">{subQ.charLimit}字以内</span>
                      </div>
                      <div className="sq-row">
                        <span className="sq-row-label">あなたの解答</span>
                        <span className="sq-answer">{subQ.answer || <em className="no-answer">未回答</em>}</span>
                      </div>
                      <div className="kw-list">
                        {subQ.keywords.map((kw) => (
                          <span key={kw} className={`kw-tag ${subQ.found.includes(kw) ? "kw-ok" : "kw-ng"}`}>
                      ...
                      <div className="sq-row commentary-row">
                        ...
                        <span className="sq-commentary">{subQ.commentary}</span>
                      </div>
                    </div>
                  ));
                } else if (q.answers) {
                  return q.answers.map((ans, ansIdx) => (
                    <div key={q.id + "_" + ansIdx} className={`score-question ${markClass(ans.mark)}`}>
                      <div className="sq-header">
                        <span className="sq-label">
                          {q.label}{q.label === "設問2" ? ["ア", "イ"][ansIdx] : ["①", "②"][ansIdx]}
                        </span>
                        <span className={`sq-mark ${markClass(ans.mark)}`}>{ans.mark}</span>
                        <span className="sq-limit">{q.charLimit}字以内</span>
                      </div>
                      <div className="sq-row">
                        <span className="sq-row-label">あなたの解答</span>
                        <span className="sq-answer">{ans.answer || <em className="no-answer">未回答</em>}</span>
                      </div>
                      <div className="kw-list">
                        {ans.keywords.map((kw) => (
                          <span key={kw} className={`kw-tag ${ans.found.includes(kw) ? "kw-ok" : "kw-ng"}`}>
                  ...
                  return (
                    <div key={q.id} className={`score-question ${markClass(q.mark)}`}>
                      <div className="sq-header">
                        <span className="sq-label">{q.label}</span>
                        <span className={`sq-mark ${markClass(q.mark)}`}>{q.mark}</span>
                        <span className="sq-limit">{q.charLimit}字以内</span>
                      </div>
                      <div className="sq-row">
                        <span className="sq-row-label">あなたの解答</span>
                        <span className="sq-answer">{q.answer || <em className="no-answer">未回答</em>}</span>
                      </div>
                      <div className="kw-list">
                        {q.keywords.map((kw) => (
                          <span key={kw} className={`kw-tag ${q.found.includes(kw) ? "kw-ok" : "kw-ng"}`}>
